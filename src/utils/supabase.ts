...
// Combined function to update timer state
export async function updateTimer(tournamentCode: string, active: boolean, remaining?: number) {
  try {
    const client = ensureSupabase();
    const updateData: any = { timer_active: active };
    
    if (remaining !== undefined) {
      updateData.timer_remaining = remaining;
    }
    ...
    if (error) throw error;
    await logSupabaseOperation('Timer updated', { code: tournamentCode, active, remaining });
  } catch (error) {
    console.error('Error updating timer:', error);
    throw error;
  }
...
// Subscribe to tournament voter count
export async function subscribeTournamentVoters(tournamentCode: string, callback: (count: number) => void) {
  try {
    const client = ensureSupabase();
    
    const subscription = client
      .channel(`tournament_voters:${tournamentCode}`)
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'tournament_connections',
    ...
    console.error('Error subscribing to tournament voters:', error);
    throw error;
  }
}

// Subscribe to match timer updates
export async function subscribeToMatchTimer(tournamentCode: string, callback: (timerData: { remaining: number, active: boolean }) => void) {
  try {
    const client = ensureSupabase();
    
    const subscription = client
      .channel(`match_timer:${tournamentCode}`)
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        (payload: any) => {
          if (payload.new) {
            callback({ 
              remaining: payload.new.timer_remaining || 0, 
              active: payload.new.timer_active || false 
            });
          }
        }
      )
    ...
    const { data } = await client
      .from('tournaments')
      .select('timer_remaining, timer_active')
      .eq('code', tournamentCode)
      .single();
      
    if (data) {
      callback({ 
        remaining: data.timer_remaining || 0, 
        active: data.timer_active || false 
      });
    }
    
    return subscription;
  } catch (error) {
    console.error('Error subscribing to match timer:', error);
    throw error;
  }
}

// Subscribe to votes
export async function subscribeToVotes(tournamentCode: string, matchId: string, callback: (votes: Record<string, number>) => void) {
  try {
    const client = ensureSupabase();
    
    const subscription = client
      .channel(`match_votes:${tournamentCode}:${matchId}`)
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        async () => {
          // Get vote tallies
          const { data } = await client.rpc('get_vote_tallies', { 
            p_match_id: matchId 
          });
...
export async function subscribeToTournamentUpdates(tournamentCode: string, callback: (tournament: any) => void) {
  try {
    const client = ensureSupabase();
    
    const subscription = client
      .channel(`tournament_updates:${tournamentCode}`)
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        async (payload: any) => {
          if (payload.new && payload.new.tournament_data) {
            callback(payload.new.tournament_data);
          }
        }
      )
      .subscribe();
    ...
    console.error('Error subscribing to tournament updates:', error);
    throw error;
  }
}

// Start match with timer
export async function startMatchWithTimer(tournamentCode: string, tournament: Tournament, duration: number): Promise<boolean> {
  try {
    const client = ensureSupabase();
...
export async function endMatchWithWinner(tournamentCode: string, tournament: Tournament, winner: Participant): Promise<boolean> {
  try {
    const client = ensureSupabase();
...
export async function updateTournamentInDB(tournamentCode: string, tournament: any) {
  try {
    const client = ensureSupabase();
    const { error } = await client
      .from('tournaments')
    ...
    if (error) throw error;
    await logSupabaseOperation('Tournament updated', { code: tournamentCode });
  } catch (error) {
...
export async function checkAllVotersVoted(tournamentCode: string, matchId: string): Promise<{
...
// Accelerate timer (reduce time)
export async function accelerateTimer(tournamentCode: string): Promise<boolean> {
  try {
    const client = ensureSupabase();
...
// Cast a vote for a participant in a match
export async function castVote(tournamentCode: string, matchId: string, participantId: string, voterId: string): Promise<void> {
  try {
    const client = ensureSupabase();
    const { error } = await client
      .from('tournament_votes')
    ...
    if (error) throw error;
    await logSupabaseOperation('Vote cast', { code: tournamentCode, matchId, participantId });
  } catch (error) {
...
export async function subscribeToTournamentStatus(tournamentCode: string, callback: (tournamentData: any) => void) {
  try {
    const client = ensureSupabase();
    
    const subscription = client
      .channel(`tournament_status:${tournamentCode}`)
      .on(
        'postgres_changes',
        {
          event: '*',
    ...
    // Also get initial tournament data
    const { data } = await client
      .from('tournaments')
    ...
    if (data && data.tournament_data) {
      callback(data.tournament_data);
    }
...
// Subscribe to match status
export async function subscribeToMatchStatus(tournamentCode: string, callback: (matchData: any) => void) {
  // This is a wrapper around subscribeToTournamentStatus
  // but only calls back when the match changes
  let lastMatchId: string | null = null;
  
  return subscribeToTournamentStatus(tournamentCode, (tournamentData) => {
    if (tournamentData.currentMatch) {
      const currentMatchId = tournamentData.currentMatch.id;
      if (currentMatchId !== lastMatchId) {
        lastMatchId = currentMatchId;
        callback(tournamentData.currentMatch);
      }
    } else {
      if (lastMatchId !== null) {
        lastMatchId = null;
