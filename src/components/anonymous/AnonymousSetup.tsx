...
const AnonymousSetup: React.FC<AnonymousSetupProps> = ({ onCreateTournament, onBack }) => {
  ...
  const handleSubmit = () => {
    ...
    if (inputMode === 'paste') {
      participants = createParticipants(parseParticipantInput(participantInput));
      ...
      participants = createParticipants(Array.from({ length: participantCount }, (_, i) => `Player ${i + 1}`));
    }
